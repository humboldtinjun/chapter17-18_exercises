"""exercise 6: A poker hand has a pair if it contains two or more cards with the same rank. Write a PokerHand method
that checks whether a hand contains a pair."""
...
class PokerHand(Hand):
    ...
    def has_pair(self):
        """Returns True if hand has a pair (two or more cards with same rank)."""
...
# Or manually add a pair for testing
bad_hand = PokerHand()
bad_hand.cards = [Card(0, 2), Card(1, 3), Card(2, 4), Card(3, 5), Card(0, 7)]
pair = deepcopy(bad_hand)
pair.put_card(Card(1, 2))
print(pair)
print("Pair (should be True):", pair.has_pair())
